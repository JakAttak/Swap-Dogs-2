...
public class CameraController : HelperFunctions {
    ...
    void rotateCamera() {
        ...
        if (panning) {
            ...
            float deltaX = speedH * 2 * Input.GetAxis("Camera1");
            float deltaY = speedV * Input.GetAxis("Camera2");
            if (Input.GetMouseButton(0)) {
                deltaX = speedH * Input.GetAxis("Mouse X");
                deltaY = speedV * Input.GetAxis("Mouse Y");
            }
            ...
            manualRotation.y += deltaX;
            manualRotation.x += deltaY;
